---
title: "Stimuli and Mouse Neuron Spikes: Mixed effect Models"
author: "WEITING LIN"
date: "`r Sys.Date()`"
output:
...
From the plots below (Figure 3 to Figure 4), we can see that the highest mean firing rate occurs when the contrast level of the right stimulus is at 1, excluding session 1. Additionally, the mean firing rate does not seem to have exaggerated fluctuations over the five levels of left stimuli in each session, except for session 1.

In general, we could learn that the values of mean firing rate in each session are different. From figure 3, it is obvious that the mean firing rate will decrease with the trials increase on the mouse.
...
From the Figure 5, we could learn that there exists the interaction effect between left side contrast levels and right side contrast levels. Because the lines in the figure 5, combinations of left side contrast levels and right side contrast levels, cross.

```{r echo=FALSE, message=FALSE, warning=FALSE,fig.align="center"}
interaction.plot(new_data$contrast_left,new_data$contrast_right,new_data$avg_firing_rate,ylab="mean firing rate",xlab="left contrast levels",trace.label = "right contrast levels",col=c("orange","green","red","blue"),main="Figure 5: interaction effect between right contrast levels and left contrast levels",cex.main=0.8)
```

We would like to determine whether there is an interaction effect between the contrast levels of the two sides in any session. Figures 6 to 10 show that the four lines in each plot intersect, indicating that each session has an interaction effect that may affect the mean firing rate.
...
In Figure 11, the QQ-plot shows that there is a heavy tail and some outliers, which are labeled with their corresponding session. Overall, it is still obvious that the distribution of the residuals follows the diagonal line. Hence, we can conclude that the residuals are normally distributed.

```{r echo=FALSE, message=FALSE, warning=FALSE,fig.align="center"}
#install.packages("lattice")
library(lattice)
#qqnorm(resid(fit.full))
qqmath(fit.full, id=0.05, main="Figur 11: QQ-plot")#id:outlier
```
...
plot(cooksD,main="Figur 12: outlier and all data points")
...
We would like to determine whether the mixed model follows homogeneity of variance. The plot below (Figure 13) shows that the residuals are not symmetric with respect to the line y=0. Therefore, we conclude that the model violates the assumption of homogeneity of variance.

```{r echo=FALSE, message=FALSE, warning=FALSE,fig.align="center"}
plot(fit.full,main="Figure 13: fitted value versus residual",xlab="fitted value",ylab = "residual")
...
plot(fit.full, resid(.) ~ fitted(.) | session, main="Figur 14: residuals among five sessions")
...
boxplot(cbind(res.P, res.D), names = c("Pearson", "Deviance"),main="Figur 15: distribution- Pearson residuals vs. deviance residuals",cex.main=0.8)
```

#### Residuals plots

The purpose is to determine if there are systematic patterns in the residuals. The plots below show that both pearson residuals plot and deviance residuals plot do not have symmetry pattern. In addition, the red curve captures this non-linear pattern; hence, the model may not capture some non-linear effects.

```{r echo=FALSE, message=FALSE, warning=FALSE,fig.align="center"}
par(mfrow=c(1,2))
plot(model$fitted.values, res.P, pch=16, cex=0.6, ylab='Pearson Residuals', xlab='Fitted Values',main="Figur 16: Pearson residuals",cex.main=0.8)
lines(smooth.spline(model$fitted.values, res.P, spar=0.9), col=2)
abline(h=0, lty=2, col='grey')
plot(model$fitted.values, res.D, pch=16, cex=0.6, ylab='Deviance Residuals', xlab='Fitted Values',main="Figur 17: Deviance residuals",cex.main=0.8)
...
plot(names(leverage), leverage, xlab="Index", type="h",main="Figure 18: influential points")
